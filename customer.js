...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const CAN_CREATE = rbacEl ? rbacEl.dataset.canCreate === "1" : (window.RbacPage && window.RbacPage.can("create"));
  const CAN_EDIT = rbacEl ? rbacEl.dataset.canEdit === "1" : (window.RbacPage && window.RbacPage.can("edit"));
  const CAN_DELETE = rbacEl ? rbacEl.dataset.canDelete === "1" : (window.RbacPage && window.RbacPage.can("delete"));
  const CAN_IMPORT = rbacEl ? rbacEl.dataset.canImport === "1" : (window.RbacPage && window.RbacPage.can("create"));
  ...
  function fetchCustomer() {
    ...
    if (!listBootstrapUsed) {
      listBootstrapUsed = true;
      const bootstrap = window.ListBootstrap && window.ListBootstrap.consume();
      if (bootstrap) {
        applyCustomerPayload(bootstrap);
