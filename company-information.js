document.addEventListener("DOMContentLoaded", () => {
    ...
    function applyCompanyCodeLock(locked, code) {
      companyCodeLocked = !!locked;
      if (!companyCodeInput) return;
      companyCodeInput.readOnly = companyCodeLocked;
      if (companyCodeLocked && code) {
        companyCodeInput.value = code;
      }
      const hint = document.getElementById("companyCodeLockHint");
      if (hint) hint.style.display = companyCodeLocked ? "" : "none";
    }
    ...
    async function loadCompanyInformation() {
      try {
        ...
        if (result.company) {
          ...
          applyCompanyCodeLock(result.company_code_locked, result.company.company_code);
        }
    ...
    function runLiveValidation() {
      ...
      let branchOk = branchRows.length > 0;
      ...
      if (!branchRows.length) {
        branchTableErr.textContent = "Add at least one branch using + Add Branch.";
        ok = false;
      } else if (!branchOk) {
    ...
    function updateBranchNumbers() {
      branchTableBody.querySelectorAll("tr").forEach((row, index) => {
    ...
    async function saveCompany() {
      ...
      try {
        ...
        const pageEl = document.querySelector(".company-page");
        const wasNewCompany = pageEl && pageEl.dataset.isNewCompany === "1";
        if (wasNewCompany || result.redirect) {
          setTimeout(() => {
