...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function clearDataRows() {
    ...
    Array.from(tbody.querySelectorAll("tr")).forEach((tr) => {
      if (tr.id !== "creditNoDataRow") tr.remove();
    });
  }
  ...
  function clampListDateYear(input) {
    if (!input) return;
    let v = (input.value || "").replace(/[^\d-]/g, "");
    if (!v) return;
    if (/(\d{4})\d+/.test(v)) {
      input.value = v.replace(/(\d{4})\d+/, "$1");
      return;
    }
    const iso = v.match(/^(\d{4,})-(\d{2})-(\d{2})$/);
    if (iso) {
      input.value = `${iso[1].slice(0, 4)}-${iso[2]}-${iso[3]}`;
      return;
    }
    const lastDash = v.lastIndexOf("-");
    if (lastDash !== -1) {
      input.value = v.slice(0, lastDash + 1) + v.slice(lastDash + 1).replace(/\D/g, "").slice(0, 4);
      return;
    }
  ...
  function restrictListDateInput(input, dateSource) {
    if (!input) return;
    input.setAttribute("title", "");
    const inYear = () => {
      const s = input.selectionStart;
      const e = input.selectionEnd;
      return input._yearLikely || (s != null && e != null && Math.max(s, e) >= 6);
    };
    input.addEventListener("focus", () => {
      input._yearTyped = 0;
      input._yearLikely = false;
      input._segmentAdvance = 0;
    });
    input.addEventListener("mousedown", (ev) => {
      input._yearLikely = ev.clientX - input.getBoundingClientRect().left > input.offsetWidth * 0.45;
      input._yearTyped = 0;
    });
    ...
    input.addEventListener("keydown", (e) => {
      const yearActive = inYear() || input._yearLikely || (input._yearTyped || 0) > 0;
      if (e.key.length === 1 && /\d/.test(e.key)) {
        if (yearActive) {
          const next = (input._yearTyped || 0) + 1;
          if (next > 4) {
            e.preventDefault();
            return;
          }
          input._yearTyped = next;
        } else {
          input._yearTyped = 0;
        }
        setTimeout(() => clampListDateYear(input), 0);
      } else if (e.key === "Backspace" || e.key === "Delete") {
        input._yearTyped = Math.max(0, (input._yearTyped || 0) - 1);
      } else if (e.key === "ArrowRight") {
        input._segmentAdvance = (input._segmentAdvance || 0) + 1;
        if (input._segmentAdvance >= 2) input._yearLikely = true;
    ...
    input.addEventListener("input", () => {
      clampListDateYear(input);
      if ((input.value || "").trim()) {
        input._yearTyped = 0;
        input._yearLikely = false;
      }
    });
    input.addEventListener("keyup", () => clampListDateYear(input));
  ...
  restrictListDateInput(fromDateInput, "from");
  restrictListDateInput(toDateInput, "to");
