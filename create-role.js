...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let isSaving = false;
    const DEPT_CREATE_URL = "/department-roles/create";
    ...
    function getPermissionRows() {
        return document.querySelectorAll(".permission-table tbody tr[data-menu]");
    }

    function collectRolePermissions() {
        const permissions = {};
        getPermissionRows().forEach((row) => {
    ...
    function permissionRowsHaveAnyChecked() {
    ...
    function updateCreateRoleButtonState() {
        ...
        const anyChecked = permissionRowsHaveAnyChecked();
        ...
        getPermissionRows().forEach(row => {
            ...
            otherCheckboxes.forEach(cb => {
                cb.addEventListener("change", () => {
                    if (cb !== viewCheckbox && cb.checked && viewCheckbox) {
                        viewCheckbox.checked = true;
                    }
                    if (cb === viewCheckbox && viewCheckbox && !viewCheckbox.checked) {
                        fullAccessCheckbox.checked = false;
                        [createCheckbox, editCheckbox, deleteCheckbox].filter(Boolean).forEach((o) => {
                            o.checked = false;
                        });
                    }
                    syncFullAccess();
    ...
    if (window.UnsavedWarning) {
        UnsavedWarning.enable({ root: ".container" });
    }
    ...
    saveBtn.addEventListener("click", (e) => {
        ...
        const userRole = header ? (header.getAttribute("data-current-role") || "").toLowerCase().replace(/\s+/g, "").replace(/_/g, "") : "";
        
        if (userRole !== "superadmin") {
            showErrorNotification("Only Super Admin can create roles.");
            return;
        }
        ...
        // PERMISSIONS CHECK
        // --------------------
        if (!permissionRowsHaveAnyChecked()) {
    ...
    confirmOk.addEventListener("click", async () => {
        ...
        try {
            ...
            if (data.success === true) {
                ...
                setTimeout(() => {
                    window.location.href = DEPT_CREATE_URL;
    ...
    cancelBtn.addEventListener("click", (e) => {
        e.preventDefault();
        if (window.UnsavedWarning) UnsavedWarning.tryLeave(DEPT_CREATE_URL);
        else window.location.href = DEPT_CREATE_URL;
    });
