...
function checkAndUpdateEditableButtons() {
    ...
    if (typeof runLiveQuotationValidation === 'function') {
        runLiveQuotationValidation();
    }
...
// ===================================================
// LIVE VALIDATION — Create Quotation (from Quotation Type onward; tiered like add customer)
// ===================================================

function setQuoteFieldError(el, message) {
    if (!el) return;
    const span = el.parentElement?.querySelector('.field-error');
    if (span) span.textContent = message || '';
    if (el.classList.contains('customer-po-duplicate')) return;
    if (message) el.classList.add('input-error');
    else el.classList.remove('input-error');
}

function quotationTier1Complete() {
    const quotationType = document.getElementById('quotationType');
    const quotationDate = document.getElementById('quotationDate');
    const expiryDate = document.getElementById('expiryDate');
    if (!quotationType?.value?.trim()) return false;
    if (!quotationDate?.value || !expiryDate?.value) return false;
    if (typeof isValidDateString === 'function') {
        if (!isValidDateString(quotationDate.value)) return false;
        if (!isValidDateString(expiryDate.value)) return false;
    }
    const qDate = new Date(quotationDate.value);
    const eDate = new Date(expiryDate.value);
    qDate.setHours(0, 0, 0, 0);
    eDate.setHours(0, 0, 0, 0);
    if (eDate < qDate) return false;
    return true;
}

function quotationTier2CustomerReady() {
    return quotationTier1Complete() && !!document.getElementById('customerSelect')?.value?.trim();
}

function quotationTier2SalesRepReady() {
    return quotationTier2CustomerReady() && !!document.getElementById('salesRep')?.value?.trim();
}

function quotationTier2CurrencyReady() {
    return quotationTier2SalesRepReady() && !!document.getElementById('currency')?.value?.trim();
}

function quotationTier2PaymentReady() {
    return quotationTier2CurrencyReady() && !!document.getElementById('paymentTerms')?.value?.trim();
}

function validateQuotationTypeLive() {
    const el = document.getElementById('quotationType');
    if (!el) return;
    if (!el.value?.trim()) {
        setQuoteFieldError(el, 'Please select a quotation type.');
    } else {
        setQuoteFieldError(el, '');
    }
}

function validateCustomerSelectLive() {
    const el = document.getElementById('customerSelect');
    if (!el) return;
    if (!quotationTier1Complete()) {
        setQuoteFieldError(el, '');
        return;
    }
    if (!el.value?.trim()) {
        setQuoteFieldError(el, 'Please select a customer.');
    } else {
        setQuoteFieldError(el, '');
    }
}

function validateCustomerPoLive() {
    const el = document.getElementById('customerPo');
    if (!el) return;
    if (!quotationTier1Complete()) {
        setQuoteFieldError(el, '');
        return;
    ...
    if (v.length > 10) {
        setQuoteFieldError(el, 'Maximum 10 characters.');
    } else {
        setQuoteFieldError(el, '');
    }
}
...
function validateSalesRepLive() {
    const el = document.getElementById('salesRep');
    if (!el) return;
    if (!quotationTier2CustomerReady()) {
        setQuoteFieldError(el, '');
        return;
    }
    if (!el.value?.trim()) {
        setQuoteFieldError(el, 'Please select a sales rep.');
    } else {
        setQuoteFieldError(el, '');
    }
}

function validateCurrencyLive() {
    const el = document.getElementById('currency');
    if (!el) return;
    if (!quotationTier2SalesRepReady()) {
        setQuoteFieldError(el, '');
        return;
    }
    if (!el.value?.trim()) {
        setQuoteFieldError(el, 'Please select a currency.');
    } else {
        setQuoteFieldError(el, '');
    }
}
...
function validatePaymentTermsLive() {
    const el = document.getElementById('paymentTerms');
    if (!el) return;
    if (!quotationTier2CurrencyReady()) {
        setQuoteFieldError(el, '');
        return;
    }
    if (!el.value?.trim()) {
        setQuoteFieldError(el, 'Please select payment terms.');
    } else {
        setQuoteFieldError(el, '');
    }
}

function validateExpectedDateLive() {
    const el = document.getElementById('expectedDate');
    if (!el) return;
    if (!quotationTier2PaymentReady()) {
        setQuoteFieldError(el, '');
        return;
    }
    if (!el.value) {
        setQuoteFieldError(el, 'Please select expected date.');
        return;
    }
    const today = new Date();
    ...
    if (selectedDate < today) {
        setQuoteFieldError(el, 'Expected date cannot be in the past.');
    } else {
        setQuoteFieldError(el, '');
    }
}

function runLiveQuotationValidation() {
    validateQuotationTypeLive();
    validateCustomerSelectLive();
    validateCustomerPoLive();
    validateSalesRepLive();
    validateCurrencyLive();
    validatePaymentTermsLive();
    validateExpectedDateLive();
}
...
function initializeAllComponents() {
    ...
    if (window.UnsavedWarning) {
        UnsavedWarning.enable({ root: ".quotation-full" });
    }
...
function initializeFormValidation() {
    ...
    const formFields = [
        ...
        customerSelect, customerPo, salesRep, currency, paymentTerms, expectedDate
    ];
    ...
    function checkFormValidity() {
        ...
        const salesRepEl = document.getElementById('salesRep');
        if (!salesRepEl || !salesRepEl.value?.trim()) return false;
    ...
    if (!validateDates()) {
        ...
        return allQuotationItemRowsQtyDiscountValid();
    }
    ...
    setTimeout(function () {
        if (typeof runLiveQuotationValidation === 'function') runLiveQuotationValidation();
        validateForm();
    }, 500);
}
...
function updateUIForStatus(status) {
    ...
    switch(displayStatus) {
        ...
        case 'expired':
            ...
            const expiryDate = document.getElementById('expiryDate')?.value;
            if (expiryDate) {
                showToast(`This quotation expired on ${formatDate(expiryDate)}`, 'warning');
            }
...
/** Inline message under Quotation Items product dropdown; show on load until a product is chosen. */
function updateQuotationItemProductSelectValidation(selectElement) {
    if (!selectElement || !selectElement.classList.contains('product-select')) return;
    const wrap = selectElement.closest('.product-select-wrap');
    const errEl = wrap
        ? wrap.querySelector('.product-select-error')
        : selectElement.parentElement.querySelector('.product-select-error');
    if (typeof isEditable === 'function' && !isEditable()) {
...
/** Line with a product selected must have qty > 0 (whole number) and discount in 0..MAX_DISCOUNT_PERCENT (required, so empty is invalid). */
function getQuotationItemQtyDiscountState(row) {
    const productSelect = row.querySelector('.product-select');
    if (!productSelect || !productSelect.value || String(productSelect.value).trim() === '') {
        return { hasProduct: false, quantityOk: true, discountOk: true, quantityInput: null, discountInput: null };
    }
    ...
    if (discRaw !== '') {
        ...
        discountOk = Number.isFinite(d) && d >= 0 && d <= MAX_DISCOUNT_PERCENT;
    }

    return { hasProduct: true, quantityOk, discountOk, quantityInput, discountInput };
}

function updateQuotationItemRowQtyDiscountValidation(row) {
    if (!row) return;
    if (typeof isEditable === 'function' && !isEditable()) {
        row.querySelectorAll('.quantity-input, .discount-input').forEach((el) => el.classList.remove('input-error'));
        return;
    }
    const s = getQuotationItemQtyDiscountState(row);
    if (!s.hasProduct) {
...
function allQuotationItemRowsQtyDiscountValid() {
    if (typeof isEditable === 'function' && !isEditable()) return true;
    const rows = document.querySelectorAll('#itemsTableBody tr');
    for (let i = 0; i < rows.length; i++) {
        const s = getQuotationItemQtyDiscountState(rows[i]);
        if (s.hasProduct && (!s.quantityOk || !s.discountOk)) return false;
    }
    return true;
}

function validateAllQuotationItemProductSelects() {
    document.querySelectorAll('#itemsTableBody tr .product-select').forEach((sel) => {
        updateQuotationItemProductSelectValidation(sel);
    });
    document.querySelectorAll('#itemsTableBody tr').forEach((row) => {
        updateQuotationItemRowQtyDiscountValidation(row);
    });
}

window.updateQuotationItemProductSelectValidation = updateQuotationItemProductSelectValidation;
...
function addNewRow() {
    ...
    newRow.innerHTML = `
        ...
        <td class="product-name-cell">
            ...
            <span class="field-error product-select-error" aria-live="polite"></span>
            </div>
        </td>
    ...
    if (newSelect) updateQuotationItemProductSelectValidation(newSelect);
    updateQuotationItemRowQtyDiscountValidation(newRow);
    ...
    if (selectedOption.value) {
        try {
            ...
            if (productData.status && productData.status.toLowerCase() !== 'active') {
                showToast(`This product is ${productData.status}`, 'error');
                selectElement.value = '';
        ...
        if (typeof updateQuotationItemProductSelectValidation === 'function') {
            updateQuotationItemProductSelectValidation(selectElement);
        }
        const rowEl = selectElement.closest('tr');
        if (rowEl && typeof updateQuotationItemRowQtyDiscountValidation === 'function') {
            updateQuotationItemRowQtyDiscountValidation(rowEl);
        }
    }
};
function disableInputsForRow(row) {
...
window.removeRow = function(rowId) {
    ...
    if (row) {
        ...
        checkAndUpdateButtons();
        checkAndUpdateEditableButtons();
        if (typeof validateAllQuotationItemProductSelects === 'function') {
            validateAllQuotationItemProductSelects();
        }
    }
};
...
function initializeButtons() {
    const QUOTATION_LIST_URL = "/quotation";

    document.getElementById("cancelBtn")?.addEventListener("click", function(e) {
        e.preventDefault();
        if (window.UnsavedWarning) UnsavedWarning.tryLeave(QUOTATION_LIST_URL);
        else window.location.href = QUOTATION_LIST_URL;
    });
...
function openApproveQuotationModal() {
    const modal = document.getElementById("approveModal");
    if (modal) modal.style.display = "flex";
}

function closeApproveQuotationModal() {
    const modal = document.getElementById("approveModal");
    if (modal) modal.style.display = "none";
}

document.getElementById("approveModal")?.addEventListener("click", (e) => {
    if (e.target.id === "approveModal") closeApproveQuotationModal();
});

document.getElementById("approveCancelBtn")?.addEventListener("click", () => {
    closeApproveQuotationModal();
});
...
function validateRequiredFields() {
    ...
    let needsQtyLine = false;
    let needsDiscountLine = false;
    rows.forEach(row => {
        const s = getQuotationItemQtyDiscountState(row);
        if (!s.hasProduct) return;
        if (!s.quantityOk) needsQtyLine = true;
        if (!s.discountOk) needsDiscountLine = true;
    });
    if (needsQtyLine) {
        missing.push(`Quantity (positive whole number) for each selected product`);
        console.log('❌ Missing: valid quantity on line items');
    }
    if (needsDiscountLine) {
        missing.push(`Discount (%) (0–${MAX_DISCOUNT_PERCENT}) for each selected product`);
        console.log('❌ Missing: valid discount on line items');
    }

    rows.forEach(row => updateQuotationItemRowQtyDiscountValidation(row));
...
function validateExpectedDate() {
    ...
    if (selectedDate < today) {
        showToast('Expected date cannot be in the past', 'warning');
...
function checkDraftRequirements() {
    ...
    const salesRepDraft = document.getElementById('salesRep');
    if (!salesRepDraft || !salesRepDraft.value?.trim()) {
        console.log("❌ Missing: Sales Rep");
        return false;
    }
    ...
    if (!allQuotationItemRowsQtyDiscountValid()) {
        console.log('❌ Missing: Quantity or discount (%) on line items');
        document.querySelectorAll('#itemsTableBody tr').forEach((row) => updateQuotationItemRowQtyDiscountValidation(row));
        return false;
    }
