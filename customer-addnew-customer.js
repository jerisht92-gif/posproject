document.addEventListener("DOMContentLoaded", () => {
    const CUSTOMER_LIST_URL = "/customer";
    ...
    if (window.UnsavedWarning) {
      UnsavedWarning.enable({ root: "#customerForm" });
    }
    ...
    if (discardBtn) {
      discardBtn.addEventListener("click", (e) => {
        e.preventDefault();
        if (window.UnsavedWarning) UnsavedWarning.tryLeave(CUSTOMER_LIST_URL);
        else window.location.href = CUSTOMER_LIST_URL;
      });
    }
...
async function loadDropdownsFromCustomerJson() {
  ...
  taxIdInput.addEventListener("blur", () => {
      ...
      if (window.GstVerify) {
        window.GstVerify.attachGstinBlur(taxIdInput, {
          beforeVerify: () => validateTaxIdLive(),
        });
      }
    }
