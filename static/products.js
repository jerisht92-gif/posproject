...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function isValidCategory(v) {
    const allowed = [
      "Electronics", "Home Decor", "Exercise", "Audio", "Drinkware",
      "Clothing", "Bedroom", "Office", "Living Room",
    ];
    v = (v || "").trim().toLowerCase();
    return allowed.map(x => x.toLowerCase()).includes(v);
  }
      ...
      tr.innerHTML = `
        ...
        <td>
          <div class="action-wrap">
            <button class="action-btn edit-btn" data-id="${p.product_id}">Edit</button>
            <button class="action-btn delete-btn" data-id="${p.product_id}">Delete</button>
          </div>
        </td>
      `;
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (delBtn) {
      ...
      if (deleteText) {
        deleteText.textContent = `Are you sure you want to delete "${id}"? This action cannot be undone.`;
      }
