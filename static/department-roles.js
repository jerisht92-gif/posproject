...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let deptSearchDebounceTimer = null;
  let departmentsFetchController = null;
  ...
  function updateNoDataRow() {
    const hasData = allDepartments.length > 0;
  ...
  function renderPage() {
    ...
    updateNoDataRow();
    updateCountsAndPages(totalItems, totalPages);
  }
  ...
  if (searchInput) {
    searchInput.addEventListener("input", () => {
      if (deptSearchDebounceTimer) clearTimeout(deptSearchDebounceTimer);
      deptSearchDebounceTimer = setTimeout(() => {
        applyFilter();
      }, 250);
    });
  }
