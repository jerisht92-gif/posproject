...
document.addEventListener("DOMContentLoaded", () => {
  ...
  editCode?.addEventListener("input", () => clrErr(editCode, codeErr));
  editName?.addEventListener("input", () => clrErr(editName, nameErr));
  // Sanitize description input: allow only letters, spaces, comma, slash, dot and &
  editDesc?.addEventListener("input", () => {
    if (!editDesc) return;
    // Remove any character that is not A-Z, a-z, space, comma, slash, dot or ampersand
    editDesc.value = editDesc.value.replace(/[^A-Za-z\s,\/.&]/g, "");
    clrErr(editDesc, descErr);
  });
  ...
  saveBtn?.addEventListener("click", () => {
    ...
    if (!code) {
      ...
      setErr(editCode, codeErr, "Code must be 2 to 10 characters.");
      hasErr = true;
    } else if (!/^[A-Za-z0-9_-]+$/.test(code)) {
      setErr(editCode, codeErr, "Code can contain only letters, numbers, - or _.");
      hasErr = true;
    }
    ...
    if (!name) {
      ...
      setErr(editName, nameErr, "Name must be 3 to 40 characters.");
      hasErr = true;
    ...
    if (!desc) {
      ...
      hasErr = true;
    } else if (desc.length < 3 || desc.length > 120) {
      setErr(editDesc, descErr, "Description must be 3 to 120 characters.");
      hasErr = true;
    ...
    fetch("/department-roles/edit", {
      ...
      .then((data) => {
        if (data.success) {
          ...
          showSuccessNotification("Data has been edited successfully");
        } else {
  ...
  confirmDel?.addEventListener("click", () => {
    ...
    fetch("/department-roles/delete", {
      ...
      .then((data) => {
        ...
        else alert("Failed to delete department: " + (data.error || "Unknown error"));
      })
      .catch((err) => {
