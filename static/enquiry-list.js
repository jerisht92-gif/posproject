...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const tableBody = document.getElementById("enquiryTable");
    ...
    let totalItems = 0;
    let totalPages = 1;
    let enquiryFetchController = null;
    ...
    function renderPagination() {
        ...
        if (showingCount) {
            showingCount.textContent = totalItems > 0 ? `${startEntry}-${end} of ${totalItems}` : "0";
        }
        if (pageInfo) pageInfo.textContent = `Page ${currentPage} of ${safeTotalPages}`;
        if (prevPageBtn) prevPageBtn.disabled = currentPage <= 1;
        if (nextPageBtn) nextPageBtn.disabled = currentPage >= safeTotalPages;
    }
    ...
    async function loadEnquiriesFromApi() {
        try {
            ...
            if (enquiryFetchController) enquiryFetchController.abort();
            enquiryFetchController = new AbortController();

            const res = await fetch(`${ENQUIRIES_API}?${params.toString()}`, {
                headers: { Accept: "application/json" },
                credentials: "same-origin",
                cache: "no-store",
                signal: enquiryFetchController.signal,
            });
            ...
            if (Array.isArray(payload.enquiries)) {
                ...
                totalItems = Number(payload.total || payload.enquiries.length || 0);
...
console.log("Length:", currentItems.length);
    ...
    function resetProductModalToViewMode() {
        if (modalTitle) modalTitle.textContent = "View Product";
        if (qtyEl) {
    ...
    if (saveBtnEl) saveBtnEl.addEventListener("click", async () => {  
        ...
        try {
            ...
            if (result.success) {
                closeProductModal();
                await loadEnquiriesFromApi();
                resetProductModalToViewMode();
                showSuccessNotification("Items saved successfully.", 3000);
            } else {
