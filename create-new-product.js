document.addEventListener("DOMContentLoaded", () => {
    let updateAddProductButtonState = () => {};
    ...
    async function fetchProductId() {
      try {
        const res = await fetch("/api/products/new-id", { credentials: "same-origin" });
        if (!res.ok) {
    ...
    const brandNameCustom = document.getElementById("brandNameCustom");
    const brandNameHidden = document.getElementById("brandNameHidden");

    function populateBrandOptions(brands) {
      if (!brandNameSelect || !Array.isArray(brands)) return;
      const seen = new Set();
      brands.forEach((name) => {
        const label = (name || "").trim();
        if (!label || seen.has(label.toLowerCase())) return;
        seen.add(label.toLowerCase());
        const exists = [...brandNameSelect.options].some(
          (o) => (o.value || "").toLowerCase() === label.toLowerCase()
        );
        if (!exists) {
          brandNameSelect.add(new Option(label, label));
        }
      });
    }

    if (brandNameSelect) {
      fetch("/api/products?page_size=1000", { credentials: "same-origin" })
        .then((res) => (res.ok ? res.json() : null))
        .then((response) => {
          if (!response?.success) return;
          const metaBrands = response?.data?.meta?.brands;
          if (Array.isArray(metaBrands) && metaBrands.length) {
            populateBrandOptions(metaBrands);
            return;
          }
          const items = response?.data?.items || [];
          populateBrandOptions(
            items.map((p) => (p.brand_name || p.brand || "").trim()).filter(Boolean)
          );
        })
        .catch(() => {});
    }

    if (brandNameSelect && brandNameCustom) {
      const brandFieldError = brandNameCustom.closest(".field")?.querySelector(".error-msg");

      brandNameSelect.addEventListener("change", () => {
        if (brandNameSelect.value === "custom") {
          brandNameSelect.style.display = "none";
          brandNameCustom.removeAttribute("hidden");
          brandNameCustom.style.display = "block";
          brandNameCustom.value = "";
          brandNameCustom.classList.remove("input-error");
          if (brandFieldError) {
            brandFieldError.textContent = "";
            brandFieldError.style.display = "none";
          }
          brandNameCustom.focus();
        } else {
          if (brandNameHidden) {
            brandNameHidden.value = (brandNameSelect.value || "").trim();
          }
          updateAddProductButtonState();
        }
      });

      brandNameCustom.addEventListener("input", () => {
        const original = brandNameCustom.value || "";
        let cleaned = original.replace(/[^A-Za-z0-9\s]/g, "");
        if (cleaned.length > 30) cleaned = cleaned.substring(0, 30);
        if (cleaned !== original) brandNameCustom.value = cleaned;
        brandNameCustom.classList.remove("input-error");
        if (brandFieldError) {
          brandFieldError.textContent = "";
          brandFieldError.style.display = "none";
      ...
      brandNameCustom.addEventListener("paste", (e) => {
        e.preventDefault();
        let cleaned = (e.clipboardData || window.clipboardData).getData("text").replace(/[^A-Za-z0-9\s]/g, "");
        if (cleaned.length > 30) cleaned = cleaned.substring(0, 30);
        brandNameCustom.value = cleaned;
        brandNameCustom.dispatchEvent(new Event("input"));
      });

      const saveBrandCustom = () => {
        const value = brandNameCustom.value.trim();
        if (!value) {
          brandNameCustom.setAttribute("hidden", "");
          brandNameCustom.style.display = "none";
          brandNameSelect.style.display = "block";
        ...
        if (value.length < 3 || !/^[A-Za-z0-9\s]+$/.test(value)) {
          if (brandFieldError) {
            brandFieldError.textContent = "Use only letters, numbers and spaces (min 3 characters).";
            brandFieldError.style.display = "block";
          }
          brandNameCustom.classList.add("input-error");
          return;
        }
        if (brandFieldError) {
          brandFieldError.textContent = "";
        ...
        brandNameCustom.classList.remove("input-error");
        let option = [...brandNameSelect.options].find(
          o => (o.value || "").toLowerCase() === value.toLowerCase()
        );
        if (!option) {
          option = new Option(value, value);
          brandNameSelect.add(option);
        }
        brandNameSelect.value = value;
        if (brandNameHidden) brandNameHidden.value = value;
        brandNameCustom.setAttribute("hidden", "");
        brandNameCustom.style.display = "none";
        brandNameSelect.style.display = "block";
        updateAddProductButtonState();
      ...
      brandNameCustom.addEventListener("blur", saveBrandCustom);
      brandNameCustom.addEventListener("keydown", e => {
        if (e.key === "Enter") {
          e.preventDefault();
          saveBrandCustom();
        }
    ...
    // ==========================
    // ✅ HSN Code — digits only, validated against hsn_code master table
    // ==========================
    const hsnCodeInput = document.getElementById("hsnCodeInput");
    const HSN_FORMAT_RE = /^\d{2,8}$/;
    const hsnMasterState = { valid: false, pending: false, lastCode: "" };
    let hsnMasterDebounce = null;
    let hsnMasterSeq = 0;

    function hsnMasterErrorEl() {
      const wrapper = hsnCodeInput?.closest(".field");
      return wrapper ? wrapper.querySelector(".error-msg") : null;
    }

    function setHsnMasterError(message) {
      const err = hsnMasterErrorEl();
      if (err) {
        err.textContent = message || "";
        err.style.display = message ? "block" : "none";
      }
      if (hsnCodeInput) {
        hsnCodeInput.classList.toggle("input-error", !!message);
      }
      if (window.GstVerify) {
        if (message) window.GstVerify.clearGstHint(hsnCodeInput);
      }
    }

    function setHsnMasterOk(description) {
      hsnMasterState.valid = true;
      setHsnMasterError("");
      if (!window.GstVerify || !hsnCodeInput || !description) return;
      const hint =
        description.length > 90
          ? `Valid HSN: ${description.slice(0, 87)}…`
          : `Valid HSN: ${description}`;
      window.GstVerify.setGstHint(hsnCodeInput, hint, "ok");
    }

    async function lookupHsnInMaster(code, { forSubmit = false } = {}) {
      const seq = ++hsnMasterSeq;
      hsnMasterState.pending = true;
      hsnMasterState.valid = false;
      if (!forSubmit && window.GstVerify) {
        window.GstVerify.setGstHint(hsnCodeInput, "Checking HSN in master list…", "warn");
      }

      try {
        const res = await fetch(
          `/api/hsn/lookup?code=${encodeURIComponent(code)}`,
          { credentials: "same-origin" }
        );
        const data = await res.json().catch(() => ({}));
        if (seq !== hsnMasterSeq) return false;

        if (data.found) {
          hsnMasterState.lastCode = code;
          setHsnMasterOk((data.description || "").trim());
          return true;
        }

        const msg =
          data.message || "HSN code not found in master list. Choose a valid HSN.";
        setHsnMasterError(msg);
        return false;
      } catch (_err) {
        if (seq !== hsnMasterSeq) return false;
        setHsnMasterError("Unable to verify HSN code right now.");
        return false;
      } finally {
        if (seq === hsnMasterSeq) {
          hsnMasterState.pending = false;
        }
      }
    }

    function scheduleHsnMasterLookup() {
      const code = (hsnCodeInput?.value || "").trim();
      hsnMasterState.valid = false;
      if (code !== hsnMasterState.lastCode) {
        hsnMasterState.lastCode = "";
      }
      if (window.GstVerify) {
        window.GstVerify.clearGstHint(hsnCodeInput);
      }
      ...
      if (!HSN_FORMAT_RE.test(code)) {
        setHsnMasterError("HSN code must be 2 to 8 digits");
        return;
      }

      setHsnMasterError("");
      clearTimeout(hsnMasterDebounce);
      hsnMasterDebounce = setTimeout(() => lookupHsnInMaster(code), 400);
    }

    if (hsnCodeInput) {
      hsnCodeInput.addEventListener("input", () => {
        let value = (hsnCodeInput.value || "").replace(/\D/g, "");
        if (value.length > 8) value = value.substring(0, 8);
        hsnCodeInput.value = value;
        scheduleHsnMasterLookup();
        if (typeof updateAddProductButtonState === "function") {
          updateAddProductButtonState();
        }
      });

      hsnCodeInput.addEventListener("keydown", (e) => {
        if ([8, 9, 27, 13, 46, 37, 38, 39, 40, 35, 36].indexOf(e.keyCode) !== -1 ||
            (e.keyCode === 65 && e.ctrlKey === true) ||
            (e.keyCode === 67 && e.ctrlKey === true) ||
            (e.keyCode === 86 && e.ctrlKey === true) ||
        ...
        if (hsnCodeInput.value.length >= 8) {
          e.preventDefault();
        }
      });

      hsnCodeInput.addEventListener("paste", (e) => {
        e.preventDefault();
        let value = (e.clipboardData || window.clipboardData).getData("text").replace(/\D/g, "");
        if (value.length > 8) value = value.substring(0, 8);
        hsnCodeInput.value = value;
        hsnCodeInput.dispatchEvent(new Event("input"));
      });

      hsnCodeInput.addEventListener("blur", () => {
        clearTimeout(hsnMasterDebounce);
        const code = (hsnCodeInput.value || "").trim();
        if (!code) {
          setHsnMasterError("");
    ...
    if (window.UnsavedWarning) {
      UnsavedWarning.enable({ root: "#createProductForm" });
    }

    if (discardBtn) {
      discardBtn.addEventListener("click", (e) => {
        e.preventDefault();
        if (window.UnsavedWarning) UnsavedWarning.tryLeave(PRODUCTS_LIST_URL);
        else window.location.href = PRODUCTS_LIST_URL;
      });
    }
    ...
    const newTaxCgst = document.getElementById("newTaxCgst");
    const newTaxSgst = document.getElementById("newTaxSgst");
    const newTaxIgst = document.getElementById("newTaxIgst");
    ...
    function syncSgstFromCgst(cgstEl, sgstEl, igstEl) {
      if (!cgstEl || !sgstEl) return;
      const cgstVal = (cgstEl.value || "").trim();
      if (cgstVal === "") {
        sgstEl.value = "";
        if (igstEl) igstEl.value = "";
        return;
      }
      sgstEl.value = cgstVal;
      const cgstNum = parseFloat(cgstVal);
      if (igstEl && !isNaN(cgstNum)) {
        igstEl.value = String(cgstNum * 2);
      }
    }

    if (newTaxSgst) newTaxSgst.readOnly = true;
    if (newTaxIgst) newTaxIgst.readOnly = true;
    ...
    function validateCreateTaxCode() {
      ...
      const cgstStr = (newTaxCgst?.value || "").trim();
      const sgstStr = (newTaxSgst?.value || "").trim();
      const igstStr = (newTaxIgst?.value || "").trim();
      const cgstNum = parseFloat(cgstStr);
      const sgstNum = parseFloat(sgstStr);
      ...
      const rateValid = (s, n) => s === "" || (!isNaN(n) && n >= 0 && n <= 100);
      const hasIntra =
        cgstStr !== "" && sgstStr !== "" &&
        rateValid(cgstStr, cgstNum) && rateValid(sgstStr, sgstNum) &&
        (cgstNum + sgstNum) > 0;
      const hasInter =
        igstStr !== "" && rateValid(igstStr, igstNum) && igstNum > 0;
      const hasValidRates = hasIntra || hasInter;
      ...
      if (hasValidName && taxCodeSelect) {
        ...
        if (exists && newTaxError) {
          newTaxError.innerText = "This tax name already exists.";
          newTaxError.style.display = "block";
        } else if (newTaxError && newTaxError.innerText === "This tax name already exists.") {
          newTaxError.innerText = "";
          newTaxError.style.display = "none";
        }
      }

      createTaxCodeBtn.disabled = !(hasValidName && hasValidRates);
    }
  
    const selectTaxCode = document.getElementById("selectTaxCode");
    const updateTaxCgst = document.getElementById("updateTaxCgst");
    const updateTaxSgst = document.getElementById("updateTaxSgst");
    if (updateTaxSgst) updateTaxSgst.readOnly = true;
    const updateTaxIgst = document.getElementById("updateTaxIgst");
    if (updateTaxIgst) updateTaxIgst.readOnly = true;
    ...
    function bindTaxRateInput(rateInput, onAfterInput) {
      if (!rateInput) return;
      const handleRateInput = () => {
        let value = rateInput.value;
      ...
      rateInput.addEventListener("keyup", validateCreateTaxCode);
      ...
      rateInput.addEventListener("paste", (e) => {
        e.preventDefault();
        const pastedText = (e.clipboardData || window.clipboardData).getData("text");
    ...
    bindTaxRateInput(newTaxCgst, () => syncSgstFromCgst(newTaxCgst, newTaxSgst, newTaxIgst));
    ...
    function validateUpdateTaxCode() {
      ...
      const cgstStr = (updateTaxCgst?.value || "").trim();
      const sgstStr = (updateTaxSgst?.value || "").trim();
      const igstStr = (updateTaxIgst?.value || "").trim();
      const cgstNum = parseFloat(cgstStr);
      const sgstNum = parseFloat(sgstStr);
      const igstNum = parseFloat(igstStr);
      ...
      updateTaxCodeBtn.disabled = !(hasSelection && hasValidRates);
    }
    ...
    if (selectTaxCode) {
      selectTaxCode.addEventListener("change", () => {
        ...
        const selectedOpt = selectTaxCode.selectedOptions[0];

        if (!val) {
          [updateTaxCgst, updateTaxSgst, updateTaxIgst].forEach((el) => {
            if (!el) return;
            el.value = "";
            el.disabled = true;
          });
        ...
        let cgstVal = selectedOpt?.dataset?.cgst || "";
        let sgstVal = selectedOpt?.dataset?.sgst || "";
        let igstVal = selectedOpt?.dataset?.igst || "";
        if (cgstVal === "" && sgstVal === "" && igstVal === "") {
          let percent = parseFloat(selectedOpt?.dataset?.percent || "");
          if (isNaN(percent) || percent <= 0) {
            const m = (val || "").match(/\((\d+(?:\.\d+)?)%\)/);
            if (m) percent = parseFloat(m[1]);
          }
          if (!isNaN(percent) && percent > 0) {
            cgstVal = String(percent / 2);
            sgstVal = String(percent / 2);
            igstVal = String(percent);
          }
        }
        if (updateTaxCgst) {
          updateTaxCgst.value = cgstVal;
          updateTaxCgst.disabled = false;
        }
        if (updateTaxSgst) {
          updateTaxSgst.disabled = false;
        }
        if (updateTaxIgst) {
          updateTaxIgst.disabled = false;
        }
        syncSgstFromCgst(updateTaxCgst, updateTaxSgst, updateTaxIgst);
        if (updateTaxCgst) updateTaxCgst.focus();
        if (updateTaxDesc) {
          updateTaxDesc.value = selectedOpt?.dataset?.desc || "";
          updateTaxDesc.disabled = false;
        }
    ...
    function bindUpdateTaxRateInput(rateInput, onAfterInput) {
      if (!rateInput) return;
      const handleRateInput = () => {
        let value = rateInput.value;
        if (/^0+(\d*)$/.test(value)) value = value.replace(/^0+/, "") || "";
      ...
      rateInput.addEventListener("input", handleRateInput);
      rateInput.addEventListener("change", handleRateInput);
      rateInput.addEventListener("keyup", validateUpdateTaxCode);
      rateInput.addEventListener("keydown", (e) => {
        if ([8, 9, 27, 13, 46, 37, 38, 39, 40, 35, 36].indexOf(e.keyCode) !== -1 ||
            (e.keyCode === 65 && e.ctrlKey === true) ||
            (e.keyCode === 67 && e.ctrlKey === true) ||
    ...
    bindUpdateTaxRateInput(updateTaxCgst, () => syncSgstFromCgst(updateTaxCgst, updateTaxSgst, updateTaxIgst));
    ...
    function loadSavedTaxCodes() {
      ...
      if (taxCodeCache.loaded) {
        taxCodeCache.items.forEach(item => {
          ...
          if (!exists) {
            ...
            if (!isNaN(cgst)) opt.dataset.cgst = String(cgst);
            if (!isNaN(sgst)) opt.dataset.sgst = String(sgst);
            if (!isNaN(igst)) opt.dataset.igst = String(igst);
      ...
      fetch("/api/product-tax-codes", { credentials: "same-origin" })
        .then(res => (res.ok ? res.json() : null))
        .then(data => {
    ...
    if (addNewTaxCodeLink) {
      addNewTaxCodeLink.addEventListener("click", (e) => {
        ...
        [newTaxCgst, newTaxSgst, newTaxIgst].forEach((el) => { if (el) el.value = ""; });
        ...
        setTimeout(() => validateCreateTaxCode(), 0);
      });
    }
      ...
      openEditTaxCodeBtn.addEventListener("click", () => {
        ...
        if (selectTaxCode && taxCodeSelect) {
          ...
          Array.from(taxCodeSelect.options).forEach(opt => {
            ...
            clone.dataset.cgst = opt.dataset.cgst || "";
            clone.dataset.sgst = opt.dataset.sgst || "";
            clone.dataset.igst = opt.dataset.igst || "";
    ...
    if (createTaxCodeBtn && taxCodeSelect) {
      createTaxCodeBtn.addEventListener("click", () => {
        ...
        const sgstNum = parseFloat(sgstStr);
        const igstNum = parseFloat(igstStr);
        const percentNum = Math.max(cgstNum + sgstNum, igstNum);
  
        if (!name) {
          if (newTaxError) {
            newTaxError.innerText = "Tax name is required";
            newTaxError.style.display = "block";
          }
          return;
        }
        ...
        if (!hasIntra && !hasInter) {
          if (newTaxError) {
            newTaxError.innerText = "Enter CGST & SGST or IGST";
            newTaxError.style.display = "block";
          }
          return;
        }
        ...
        if (
          (cgstStr !== "" && !rateValid(cgstStr, cgstNum)) ||
          (sgstStr !== "" && !rateValid(sgstStr, sgstNum)) ||
          (igstStr !== "" && !rateValid(igstStr, igstNum))
        ) {
          if (newTaxError) {
            newTaxError.innerText = "Tax rates must be between 0 and 100";
            newTaxError.style.display = "block";
          }
          return;
        }
        ...
        // Update UI immediately (optimistic update)
          ...
          opt.dataset.cgst = cgstNum.toString();
          opt.dataset.sgst = sgstNum.toString();
          opt.dataset.igst = igstNum.toString();
    ...
    if (updateTaxCodeBtn && taxCodeSelect && selectTaxCode) {
      updateTaxCodeBtn.addEventListener("click", () => {
        ...
        if (cgstStr === "" || sgstStr === "" || igstStr === "") {
          if (updateTaxError) {
            updateTaxError.innerText = "CGST, SGST and IGST are required";
            updateTaxError.style.display = "block";
          }
          return;
        }
  
        const isValidRate = (s, n) => s !== "" && !isNaN(n) && n >= 0 && n <= 100;
        if (!isValidRate(cgstStr, cgstNum) || !isValidRate(sgstStr, sgstNum) || !isValidRate(igstStr, igstNum)) {
          if (updateTaxError) {
            updateTaxError.innerText = "CGST, SGST and IGST must be between 0 and 100";
            updateTaxError.style.display = "block";
          }
          return;
        }

        if ((cgstNum + sgstNum) <= 0 && igstNum <= 0) {
          if (updateTaxError) {
            updateTaxError.innerText = "Enter CGST & SGST or IGST";
            updateTaxError.style.display = "block";
          }
          return;
        }
    ...
    if (confirmTaxCodeDeleteBtn && taxCodeSelect && selectTaxCode) {
      confirmTaxCodeDeleteBtn.addEventListener("click", () => {
        ...
        [updateTaxCgst, updateTaxSgst, updateTaxIgst].forEach((el) => { if (el) el.value = ""; });
    ...
    function findExistingUomOption(name) {
      if (!uomSelect || !name) return null;
      const key = name.trim().toLowerCase();
      return (
        Array.from(uomSelect.options).find(
          (o) => (o.value || "").trim().toLowerCase() === key
        ) || null
      );
    }
    ...
    function validateCreateUom() {
      ...
      const itemsStr = String(newUomItems?.value ?? "").trim();
      const itemsNum = parseInt(itemsStr, 10);

      const hasValidName = !!(name && UOM_NAME_REGEX.test(name));
      const hasValidItems =
        itemsStr !== "" && !isNaN(itemsNum) && itemsNum >= 1 && itemsNum <= 10000;

      const existingOpt = hasValidName ? findExistingUomOption(name) : null;
      ...
      if (name && !UOM_NAME_REGEX.test(name)) {
        if (newUomError) {
          newUomError.innerText =
            "UOM name must be 3–50 characters (letters, numbers, spaces, hyphen).";
          newUomError.style.display = "block";
        }
      } else if (itemsStr && !hasValidItems) {
        if (newUomError) {
          newUomError.innerText = "No. of items must be between 1 and 10,000.";
          newUomError.style.display = "block";
        }
      } else if (existingOpt && hasValidName && hasValidItems) {
        if (newUomError) {
          newUomError.innerText =
            `"${existingOpt.value}" already exists. Click Create to select it.`;
          newUomError.style.display = "block";
        }
      }

      // Enable when name + items are valid (Create selects existing UOM if duplicate)
      createUomBtn.disabled = !(hasValidName && hasValidItems);
      createUomBtn.dataset.useExisting = existingOpt ? "1" : "";
    }
    ...
    if (newUomItems) {
      newUomItems.addEventListener("input", (e) => {
        // Remove any non-digit characters
        let value = String(newUomItems.value ?? "").replace(/\D/g, "");
        // Limit to 10 characters
        if (value.length > 10) {
          value = value.substring(0, 10);
        }
      ...
      newUomItems.addEventListener("change", () => {
        validateCreateUom();
      });
    ...
    if (createUomBtn && uomSelect) {
      createUomBtn.addEventListener("click", () => {
        ...
        const existingOpt = findExistingUomOption(val);

        // Duplicate UOM — select existing instead of creating again
        if (existingOpt) {
          uomSelect.value = existingOpt.value;
          ...
          hideModal(addUomModal, addNewUomLink);
          validateCreateUom();
          showSuccessNotification(`UOM "${existingOpt.value}" selected.`);
          return;
        }
    ...
    if (createSizeBtn && sizeMainSelect && newSizeInput) {
      ...
      if (form) { 
        ...
        const requiredFields = [
          ...
          { selector: "#brandNameSelect",   msg: "Brand name is required" },
          ...
          { selector: "input[name='hsn_code']",       msg: "HSN code is required" },
        ...
        function attachRealtimeRequired(selector, message) {
          ...
          if (isSelect) {
            ...
            if (el.id === "brandNameSelect" && value === "custom") {
              value = (brandNameCustom?.value || "").trim();
            }
            if (!value) {
        ...
        if (brandNameCustom && brandNameSelect) {
          ["input", "blur"].forEach(evt => {
            brandNameCustom.addEventListener(evt, () => {
              let value = (brandNameSelect.value || "").trim();
              if (value === "custom") value = (brandNameCustom.value || "").trim();
              if (!value) {
                showFieldError(brandNameSelect, "Brand name is required");
              } else {
                clearFieldError(brandNameSelect);
              }
              updateAddProductButtonState();
            });
          });
        ...
        updateAddProductButtonState = function () {
          if (!submitBtn) return;

          if (brandNameSelect && brandNameHidden) {
            let bv = (brandNameSelect.value || "").trim();
            if (bv === "custom") bv = (brandNameCustom?.value || "").trim();
            brandNameHidden.value = bv;
          }
          ...
          if (hsnCodeInput) {
            const hsn = (hsnCodeInput.value || "").trim();
            if (!hsn || !HSN_FORMAT_RE.test(hsn) || !hsnMasterState.valid || hsnMasterState.pending) {
              allValid = false;
            }
          }
          ...
          if (taxCodeSelect) {
            ...
            if (taxText && percentStr && (isNaN(percentNum) || percentNum < 1 || percentNum > 100)) {
              allValid = false;
            }
          }
        ...
        form.addEventListener("submit", async (e) => {
          ...
          if (hsnCodeInput) {
            const hsnVal = (hsnCodeInput.value || "").trim();
            if (!hsnVal) {
              isValid = false;
              showFieldError(hsnCodeInput, "HSN code is required");
            } else if (!HSN_FORMAT_RE.test(hsnVal)) {
              isValid = false;
              showFieldError(hsnCodeInput, "HSN code must be 2 to 8 digits");
            } else if (!hsnMasterState.valid || hsnMasterState.lastCode !== hsnVal) {
              const dbOk = await lookupHsnInMaster(hsnVal, { forSubmit: true });
              if (!dbOk) {
                isValid = false;
                showFieldError(
                  hsnCodeInput,
                  hsnMasterErrorEl()?.textContent || "HSN code not found in master list"
                );
              } else {
                clearFieldError(hsnCodeInput);
              }
            } else {
              clearFieldError(hsnCodeInput);
            }
          ...
          const brandCustom = document.getElementById("brandNameCustom");
          const brandHidden = document.getElementById("brandNameHidden");
          if (brandSel) {
            brandVal = (brandSel.value || "").trim();
            if (brandVal === "custom") {
              brandVal = (brandCustom?.value || "").trim();
            }
          }
          if (brandHidden) brandHidden.value = brandVal;
          const formData = new FormData(form);
          formData.delete("brand_name");
          if (brandVal) formData.append("brand_name", brandVal);
          ...
          fetch("/save-product", {
            ...
            .then(data => {
              if (data.success) {
                ...
                window.location.href = PRODUCTS_LIST_URL;
